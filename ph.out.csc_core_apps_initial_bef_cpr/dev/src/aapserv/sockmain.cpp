...
#define S_ANZ_LEN  4
//#define LOCAL_WINDEBUG_PORT 56203
...
extern "C" void WriteLogInfo( const char *err_str, int err_num, int fehler_art )
{
	std::stringstream s;
	s << "rc=" << err_num << " " << err_str;

	if ( fehler_art > -1 )
		BLOG_INFO(libcsc::LoggerPool::getLoggerSession(), s.str());
	else if ( fehler_art == -1 )
...
/*static void prozess_ende() ausgesternt am 30.12.96 alex */
void prozess_ende()
{
	/* Prozess beendet */
...
int main(int argc, char **argv)
{
	...
	WriteLogInfo("Prozess-Start", -1, 0);
...
#ifndef LOCAL_WINDEBUG_PORT
	env = getenv( "PORTNR" );
	if (env == NULL)
	{
		error_exit("No PORTNR set!");
	}
	else
...
#else
	server.sin_port = htons(LOCAL_WINDEBUG_PORT);
#endif
