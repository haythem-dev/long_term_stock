...
/******************************************************************************/
/* d:\prj\AA\UNIX\aapserv\src\RangeList ***************************************/
/******************************************************************************/
...
class CRANGELIST : public ppDadeVirtual
{
	...
	private:
		...
		int  CursorPos;
		int  AnzPos;
		vector<rangelistS> *pl; // pointer to class list
	...
	public:
		...
		bool 		SetList(const unsigned int i) { if (GetSize() <= i) return false; vector<rangelistS>::iterator itPtr = pl->begin() + i; *itPtr = s; return true; }
		bool 		InsList() { if ( !pl ) pl = new vector<rangelistS>; Strip(s); rangelistS c = s; pl->insert(pl->end(),c); AnzPos++; return true; }
		bool 		DelList(const unsigned int i) { if (GetSize() <= i) return false; vector<rangelistS>::iterator itPtr = pl->begin() + i; if ( itPtr < pl->end() ) pl->erase(itPtr); if ( --AnzPos < 0 ) AnzPos = 0; return true; }
		vector<rangelistS>::iterator begin(){ if (pl) return pl->begin(); else return NULL; };
		vector<rangelistS>::iterator end(){ if (pl) return pl->end(); else return NULL; };
		...
		const rangelistS& GetStruct() const { return s; }
	...
	public:
		...
		{
			#ifdef WIN32
			rc = ::ServerNr(pos, fkt_nr, (void *)&s, RANGELIST_BES, RANGELIST_ANZ, error_msg);
			#else
			rc = ::Server(fkt_nr, (void *)&s, RANGELIST_BES, RANGELIST_ANZ, error_msg);
			#endif
	...
	private:
		...
		int UniqueServerCall(int FncNr,int pos=0)
		{
			...
			rc = ::ServerNr(pos, FncNr, (void *)&s, RANGELIST_BES, RANGELIST_ANZ, error_msg);
			#else
