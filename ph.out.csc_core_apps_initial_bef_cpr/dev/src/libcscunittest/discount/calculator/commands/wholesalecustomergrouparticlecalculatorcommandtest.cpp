...
FIXTURE_TEST_START( CommonCalculatorCommandFixture, WholesaleCustomerGroupArticle_calculate_validPercent_givenPercent )
{
	DiscountBaseValuePtr percent( getPercent() );
	addDiscountBaseValue( percent );

	WholesaleCustomerGroupArticleCalculatorCommand cmd( CT_ROOT, CT_WH_CUSTOMER_GROUP_ARTICLE_TYPE );
	...
	CHECK_EQUAL( CT_WH_CUSTOMER_GROUP_ARTICLE_TYPE, res.getDiscountResult( 1 ).discountPercentOrigin );
}
FIXTURE_TEST_END


FIXTURE_TEST_START( CommonCalculatorCommandFixture, WholesaleCustomerGroupArticle_log_validFixedPrice_validResult )
{
	DiscountBaseValuePtr percent( getPercent() );
	addDiscountBaseValue( percent );
	...
	CHECK_EQUAL( logger->getCalcResult( CT_WH_CUSTOMER_GROUP_ARTICLE_TYPE ), res );
}
FIXTURE_TEST_END
