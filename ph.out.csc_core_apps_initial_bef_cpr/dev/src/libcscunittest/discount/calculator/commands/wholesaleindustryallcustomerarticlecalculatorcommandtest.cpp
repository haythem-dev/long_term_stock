...
FIXTURE_TEST_START( CommonCalculatorCommandFixture, WholesaleIndustryAllCustomerArticle_calculate_validPercent_givenPercent )
{
	DiscountBaseValuePtr percent( getPercent() );
	addDiscountBaseValue( percent );

	WholesaleIndustryAllCustomerArticleCalculatorCommand cmd( CT_ROOT, CT_WH_INDUSTRY_ALL_CUSTOMER_ARTICLE_TYPE );
	...
	CHECK_EQUAL( CT_WH_INDUSTRY_ALL_CUSTOMER_ARTICLE_TYPE, res.getDiscountResult( 1 ).discountPercentOrigin );
}
FIXTURE_TEST_END


FIXTURE_TEST_START( CommonCalculatorCommandFixture, WholesaleIndustryAllCustomerArticle_log_validPercent_validResult )
{
	DiscountBaseValuePtr percent( getPercent() );
	addDiscountBaseValue( percent );
	...
	CHECK_EQUAL( logger->getCalcResult( CT_WH_INDUSTRY_ALL_CUSTOMER_ARTICLE_TYPE ), res );
}
FIXTURE_TEST_END
