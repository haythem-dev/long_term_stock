#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary csc_corpha for presentation only.            
 ###############################################################################################
 ...
 readonly BINARY=csc_corpha.bin
 ...
 configfile="";
 orderfilter="";
 typeset -i test_flag=0;
 typeset -i nomail_flag=0;
 typeset -i extendedlogging_flag=0;
 ...
 readonly ORDERFILTER_OPTION="-orderfilter";
 readonly TEST_OPTION="-test";
 readonly NOMAIL_OPTION="-nomail";
 readonly EXTENDEDLOGGING_OPTION="-extendedlogging";
 ...
 function print_local_var
 {
     ...
     printf "%-30s%-90s\n" "\$ORDERFILTER_OPTION:" ">$ORDERFILTER_OPTION<"
	 printf "%-30s%-90s\n" "\$TEST_OPTION:" ">$TEST_OPTION<"
	 printf "%-30s%-90s\n" "\$NOMAIL_OPTION:" ">$NOMAIL_OPTION<"
	 printf "%-30s%-90s\n" "\$EXTENDEDLOGGING_OPTION:" ">$EXTENDEDLOGGING_OPTION<"
 ...
 function eval_parameters
 {
 	 ...
 	 do
		 ...
		 if [[ "$1" == "$ORDERFILTER_OPTION" ]]; then
		     shift;
			 if (( $# > 0 )) ; then
			     orderfilter=$1;
				 export CSC_CORPHA_FILTER=$1
				 shift;
				 continue;
		     else
			     break;
 		 ...
 		 if [[ "$1" == "$EXTENDEDLOGGING_OPTION" ]]; then
 			 shift;
			 extendedlogging_flag=1;
			 continue;
 		 fi
	 ...
	 #--------------------------#
 	 ...
 	 if [[ "${orderfilter}" == "" ]] || (( 1 != `check_string_parameter "${ORDERFILTER_OPTION}" "${orderfilter}"` )); then
              printStartStop "${STARTMSG}"
              rc=12;
              echo "${PROCNAME}: Parameter value missed or invalid value \${orderfilter}: >${orderfilter}<!" | tee -a ${LOG_RESOURCE}
              echo ${USAGE} | tee -a ${LOG_RESOURCE}
              printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
              exit ${rc};
 	 fi
 ...
 function check_string_parameter
 {
 	 ...
 	 if [[ "${_OPTION}" == "${ORDERFILTER_OPTION}" ]]; then
 		 # do the possibly necessary checks with parameter ${_ARG}
 		 # e.g. database server validity: [a-z]*([a-z]|[1-9])@+([a-z]|[1-9]|[_])_@(shm|tcp)
 		 _rc=1;
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 if (( ${test_flag} == 1 )); then
 		 _parameters="${_parameters} $TEST_OPTION ";
 	 fi

 	 if (( ${nomail_flag} == 1 )); then
 		 _parameters="${_parameters} $NOMAIL_OPTION ";
 	 fi

 	 if (( ${extendedlogging_flag} == 1 )); then
 		 _parameters="${_parameters} $EXTENDEDLOGGING_OPTION ";
 	 fi
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $BRANCHNO_OPTION $branchno $CONFIGFILE_OPTION $configfile $ORDERFILTER_OPTION $orderfilter"
 	 else
 		 _parameters="$BRANCHNO_OPTION $branchno $CONFIGFILE_OPTION $configfile $ORDERFILTER_OPTION $orderfilter";
 	 fi
 ...
 . $(dirname $0)/../vars/csc_corpha.vars;
 
 binextension=${branchno}
