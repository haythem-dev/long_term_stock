...
# set -x #zeigt jede Zeile vor ihrer Ausfuehrung jedoch nach ihren Ersetzungen an (xtrace);
 ...
 #BINARY NAMES
 readonly BINARY=csc_prune.pl
 readonly RUNTIME_BINARY=/usr/bin/perl
 
 process="${PROC_PATH}/${BINARY}"
 
 full_process="${RUNTIME_BINARY}"
 ...
 function eval_parameters
 {
 	...
 	if [[ "${dbname}" == "" ]] then
 	     printStartStop "${STARTMSG}"
 	    rc=12;
 	    echo "${PROCNAME}: Parameter value missed \${dbname}: >${dbname}<!" | tee -a ${LOG_RESOURCE}
 	    echo ${USAGE} | tee -a ${LOG_RESOURCE}
 	    printStartStop "${STOPMSG}" "with Exit-Code: <${rc}>"
 	    exit ${rc};
	fi
...
# 	 fi
 ...
 #		 _parameters="$BRANCHNO_OPTION $branchno";
	...
	#	fi
 	 ...
 	 if (( ${#_parameters} > 0 )); then
		 ...
		 #take this out if status process has learned to handle scripts ;)
		 full_process="${full_process} ${process}"
 	 fi
 ...
 #======================================================#
 if [[ "$start" == "YES" ]]; then
    echo "checking ${full_process}"
    msg=`status_process "${full_process}"`
    checkOtherProcess=$?;
    if (( $checkOtherProcess == 0 )); then
		msg=`start_process "${process}"`
		rc=$?;
		echo "$msg" | tee -a ${LOG_RESOURCE}
    else
		echo "can't start ${process}, it's still running..." | tee -a ${LOG_RESOURCE}
		echo "$msg" | tee -a ${LOG_RESOURCE}
    fi
