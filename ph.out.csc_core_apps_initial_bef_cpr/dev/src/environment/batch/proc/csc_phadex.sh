#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary csc_phadex for presentation only.            
 ###############################################################################################
 ...
 readonly BINARY=csc_phadex.bin
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $COUNTRY_OPTION $country $BRANCHNO_OPTION $branchno $CONFIGFILE_OPTION $configFile"
 	 else
 		 _parameters="$COUNTRY_OPTION $country $BRANCHNO_OPTION $branchno $CONFIGFILE_OPTION $configFile";
 	 fi
 ...
 . $(dirname $0)/../vars/csc_phadex.vars;
