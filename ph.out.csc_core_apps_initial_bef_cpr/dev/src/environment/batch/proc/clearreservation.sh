#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the exemplar binary clearreservation for presentation only.      
 ###############################################################################################
 ...
 readonly BINARY=clearreservation.bin
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "${vz}" == "" ]] || (( 1 != `check_numeric_parameter "${VZ_OPTION}" "${vz}"` )) || [[ "${vz}" == "0" ]]; then
 		 printStartStop "${STARTMSG}"
 		 rc=12;
 ...
 . $(dirname $0)/clearreservation.option
 ...
 #. $(dirname $0)/../vars/clearreservation.vars;

 activate_test_binary;
 check_existence_of_binary ${process};
 assembling_process;
 print_msg "$STARTMSG" "$*"
 export CSC_BRANCHNO=$vz
