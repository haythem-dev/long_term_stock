// PageTabCtrlMultiLang.cpp : implementation file
//
#include "stdafx.h"
...
CPageTabCtrlMultiLang::CPageTabCtrlMultiLang()
{
    m_pCurrentPage= NULL;
    m_bCalled = false;
    m_bInit = false;
...
BEGIN_MESSAGE_MAP(CPageTabCtrlMultiLang, CTabCtrl)
    ON_NOTIFY_REFLECT(TCN_SELCHANGE, OnSelchange)
END_MESSAGE_MAP()

BOOL CPageTabCtrlMultiLang::PreTranslateMessage(MSG* pMsg)
{
    int nCurSel;
...
/////////////////////////////////////////////////////////////////////////////
// CPageTabCtrlMultiLang message handlers

void CPageTabCtrlMultiLang::AddPage(CTabPageMultiLang* pPage)
{
    ASSERT(pPage != NULL);
    ...
    m_pCurrentPage = (CTabPageMultiLang*)m_PageArray[0];
}

bool CPageTabCtrlMultiLang::ChangePage(int nPageTo, int nPageFrom)
{
    if (nPageFrom < 0)
    {
        nPageFrom = 0;
        m_pCurrentPage = (CTabPageMultiLang*)m_PageArray[nPageFrom];
        m_pCurrentPage->SetActiveWindow();
    }
    if (nPageTo != -1)
    {
        CTabPageMultiLang* pPageTo = (CTabPageMultiLang*)m_PageArray[nPageTo];
        if (pPageTo->m_bEnabled)
        {
            // Hide the current window and then change to the current window.
            m_pCurrentPage->ShowWindow(FALSE);
            m_pCurrentPage = (CTabPageMultiLang*)m_PageArray[nPageTo];
            m_pCurrentPage->ShowWindow(TRUE);
            NMHDR* pNMHDR = new NMHDR;
            pNMHDR->hwndFrom = m_pCurrentPage->GetSafeHwnd();
            pNMHDR->code = PSN_SETACTIVE;
            ((CTabPageMultiLang*)m_pCurrentPage)->SendMessage(WM_NOTIFY,0,(LPARAM)pNMHDR);
            delete pNMHDR;
...
void CPageTabCtrlMultiLang::OnSelchange(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
...
void CPageTabCtrlMultiLang::FindSizeOfTabWindow(CWnd* pWndP, LPRECT pRectP)
{
    ASSERT(pWndP  != NULL);
    ASSERT(pRectP != NULL);
...
BOOL CPageTabCtrlMultiLang::AddItem(TC_ITEM* pTabCtrlItemP)
{
...
CTabPageMultiLang* CPageTabCtrlMultiLang::GetPage(const int& nIndex)
{
    if (nIndex >= GetItemCount())
        return NULL;

    return (CTabPageMultiLang*)m_PageArray[nIndex];
}

bool CPageTabCtrlMultiLang::IsChanged()
{
