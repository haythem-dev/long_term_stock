...
#define REBATEPRICE_ANZ ( sizeof(REBATEPRICE_BES) / sizeof(struct buf_desc) )
#endif
...
/* original struct of REBATEPRICE *********************************************/
...
private:
    ...
    int  CursorPos;
    int  AnzPos;
    vector<rebatepriceS> lst; // class list
...
public:
    ...
    bool         SetList(const unsigned int i) {
                     if (GetSize() <= i)
                         return false;
                     vector< rebatepriceS >::iterator it = lst.begin() + i;
                     *it = s;
                     return true;
                 }
    bool         InsList() {
...
public:
    ...
    #ifdef WIN32
        rc = ::ServerNr(pos, fkt_nr, (void *)&s, REBATEPRICE_BES, (int)REBATEPRICE_ANZ, error_msg);
    #else
        rc = ::Server(fkt_nr, (void *)&s, REBATEPRICE_BES, (int)REBATEPRICE_ANZ, error_msg);
    #endif
        if (rc < 0)
...
private:
    ...
    int UniqueServerCall(int FncNr, int pos = 0) {
       ...
       rc = ::ServerNr(pos, FncNr, (void *)&s, REBATEPRICE_BES, (int)REBATEPRICE_ANZ, error_msg);
    #else
       pos = pos;
