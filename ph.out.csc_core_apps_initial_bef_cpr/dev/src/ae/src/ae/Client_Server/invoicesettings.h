...
#define INVOICESETTINGS_ANZ ( sizeof(INVOICESETTINGS_BES) / sizeof(struct buf_desc) )
#endif
...
/* original struct of INVOICESETTINGS *****************************************/
...
private:
    ...
    int  CursorPos;
    int  AnzPos;
    vector<invoicesettingsS> lst; // class list
...
public:
    ...
    bool         SetList(const unsigned int i) {
                     if (GetSize() <= i)
                         return false;
                     vector< invoicesettingsS >::iterator it = lst.begin() + i;
                     *it = s;
                     return true;
                 }
    bool         InsList() {
                     Strip(s);
                     invoicesettingsS c = s;
    ...
    vector<invoicesettingsS>::iterator
...
public:
    ...
    #ifdef WIN32
        rc = ::ServerNr(pos, fkt_nr, (void *)&s, INVOICESETTINGS_BES, (int)INVOICESETTINGS_ANZ, error_msg);
    #else
        rc = ::Server(fkt_nr, (void *)&s, INVOICESETTINGS_BES, (int)INVOICESETTINGS_ANZ, error_msg);
    #endif
        if (rc < 0)
...
private:
    ...
    int UniqueServerCall(int FncNr, int pos = 0) {
       ...
       rc = ::ServerNr(pos, FncNr, (void *)&s, INVOICESETTINGS_BES, (int)INVOICESETTINGS_ANZ, error_msg);
    #else
       pos = pos;
