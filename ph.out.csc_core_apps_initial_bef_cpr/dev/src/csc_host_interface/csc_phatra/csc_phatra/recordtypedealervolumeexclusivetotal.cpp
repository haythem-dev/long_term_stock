...
RecordTypeDealerVolumeExclusiveTotal::RecordTypeDealerVolumeExclusiveTotal()
{
}
...
void RecordTypeDealerVolumeExclusiveTotal::doProcess( const core::TransactionRecordBasePtr record ) const
{
    METHODNAME_DEF( RecordTypeDealerVolumeExclusiveTotal, process );
	...
	try
	{
		...
		dealerVolumne.SetVolume2( tmpMoney );
		...
		else
		{
			...
			m_CSC_Logger->log(log4cplus::WARN_LOG_LEVEL, "text record process pxverbund rollback - dealer volumne exclusive total put failed", record );
		}	
	}
	catch( basar::Exception& ex )
	{
		// TODO
		m_CSC_Logger->logBasarException("RecordTypeDealerVolumeExclusiveTotal::doProcess-text basar exception caught", ex, record );
		//throw;
	}
	catch( ... )
	{
		// TODO
		m_CSC_Logger->logGenericException("RecordTypeDealerVolumeExclusiveTotal::doProcess...-text generic exception caught", record );
		//throw;
	}	
}
