...
namespace discount
{
WholesaleAllCustomerManufacturerCalculatorCommand::WholesaleAllCustomerManufacturerCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleAllCustomerManufacturerCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleAllCustomerManufacturerCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale, universal customer / customer group, special manufacturer
