...
namespace discount
{
WholesaleIndustryCustomerManufacturerCalculatorCommand::WholesaleIndustryCustomerManufacturerCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleIndustryCustomerManufacturerCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleIndustryCustomerManufacturerCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale industry, special customer, special manufacturer
