...
namespace discount
{
WholesaleCustomerArticleGroupCalculatorCommand::WholesaleCustomerArticleGroupCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleCustomerArticleGroupCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleCustomerArticleGroupCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale, special customer, special article group
