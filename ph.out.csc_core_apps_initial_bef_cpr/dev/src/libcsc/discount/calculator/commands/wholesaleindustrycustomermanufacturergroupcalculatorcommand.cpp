...
namespace discount
{
WholesaleIndustryCustomerManufacturerGroupCalculatorCommand::WholesaleIndustryCustomerManufacturerGroupCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleIndustryCustomerManufacturerGroupCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleIndustryCustomerManufacturerGroupCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale industry, special customer, special manufacturer group
