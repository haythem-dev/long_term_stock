...
namespace discount
{
WholesaleAllCustomerArticleGroupCalculatorCommand::WholesaleAllCustomerArticleGroupCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleAllCustomerArticleGroupCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleAllCustomerArticleGroupCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale, universal customer / customer group, special article group
