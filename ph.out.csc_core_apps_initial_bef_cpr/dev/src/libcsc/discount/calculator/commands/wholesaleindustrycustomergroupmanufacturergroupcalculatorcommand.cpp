...
namespace discount
{
WholesaleIndustryCustomerGroupManufacturerGroupCalculatorCommand::WholesaleIndustryCustomerGroupManufacturerGroupCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleIndustryCustomerGroupManufacturerGroupCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleIndustryCustomerGroupManufacturerGroupCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale industry, special customer group,  special manufacturer group
