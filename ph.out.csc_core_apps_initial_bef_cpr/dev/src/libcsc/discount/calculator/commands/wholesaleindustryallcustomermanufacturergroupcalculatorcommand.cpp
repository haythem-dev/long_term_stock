...
namespace discount
{
WholesaleIndustryAllCustomerManufacturerGroupCalculatorCommand::WholesaleIndustryAllCustomerManufacturerGroupCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleIndustryAllCustomerManufacturerGroupCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleIndustryAllCustomerManufacturerGroupCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale industry, universal customer / customer group,  special manufacturer group
