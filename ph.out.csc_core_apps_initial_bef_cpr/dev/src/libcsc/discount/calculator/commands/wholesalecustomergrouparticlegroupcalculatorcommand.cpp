...
namespace discount
{
WholesaleCustomerGroupArticleGroupCalculatorCommand::WholesaleCustomerGroupArticleGroupCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleCustomerGroupArticleGroupCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleCustomerGroupArticleGroupCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale, special customer group, special article group
