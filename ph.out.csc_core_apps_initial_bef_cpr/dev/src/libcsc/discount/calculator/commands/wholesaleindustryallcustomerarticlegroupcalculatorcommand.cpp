...
namespace discount
{
WholesaleIndustryAllCustomerArticleGroupCalculatorCommand::WholesaleIndustryAllCustomerArticleGroupCalculatorCommand( 
...
const DiscountCalculatorResult & WholesaleIndustryAllCustomerArticleGroupCalculatorCommand::calculate() const
{
	CALC_BLOG_TRACE_METHOD( LoggerPool::getLoggerDiscountCalc(), "WholesaleIndustryAllCustomerArticleGroupCalculatorCommand::calculate()" );
	...
	for( ; iter != list.end(); ++iter ) 
	{
		...
		for( iterClassification = classificationPtr->begin(); iterClassification != classificationPtr->end(); ++iterClassification ) 
		{
			...
			// this calculator command only takes discountspec = wholesale industry, universal customer / customer group, special article group
