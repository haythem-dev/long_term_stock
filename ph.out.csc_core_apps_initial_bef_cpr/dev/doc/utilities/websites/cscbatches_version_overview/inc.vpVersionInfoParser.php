...
var_dump( parseVersionInfos($content) );
*/
function parseVersionInfos($vpinfo)
{
		$retval = array();		
        $retval['git_version']    = "n/a";
        $retval['git_commit']     = "n/a";
        $retval['git_status']     = "n/a";
        $retval['git_tag']        = "n/a";
        $retval['git_branch']     = "n/a";
        $retval['git_origin']     = "n/a";
        $retval['git_date']       = "n/a";
        $retval['git_author']     = "n/a";  
        
		$lines = explode( "\n", $vpinfo );               
        
        $lastLineWasVersionNoHeader = false;
		...
		{					
			foreach( $lines as $line )
			{
                ...
                if( $lastLineWasVersionNoHeader == true )
                {
                    $retval['git_version'] = trim($line);
                    $lastLineWasVersionNoHeader = false;
                }
                if( substr($line, 0, 25) == 'Version-No. of programme:' ) $lastLineWasVersionNoHeader = true;
