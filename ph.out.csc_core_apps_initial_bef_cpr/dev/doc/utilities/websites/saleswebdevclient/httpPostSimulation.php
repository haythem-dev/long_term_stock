...
{	
	...
	//
	//----------------------------------------------------------------------------------------------------------------
	static function post( $strLink, $strUser, $strPasswort, $iTimeout = 700 )
	{
		$objCurl = curl_init();		
	...
	static function postData( $strLink, $strUser, $strPasswort, $strXML, $iTimeout = 200 ) //, $Proxy = 'proxy:8080' )
	{
		...
		$xdata = str_replace( "UNIQUEID", substr( uniqid(md5(rand()), true), 0, 9), $strXML );
		...
		curl_setopt( $objCurl, CURLOPT_HEADER, 0 );
		curl_setopt( $objCurl, CURLOPT_POST, 1 );
		curl_setopt( $objCurl, CURLOPT_POSTFIELDS, $xdata );
		curl_setopt( $objCurl, CURLOPT_TIMEOUT, (int)$iTimeout );		
