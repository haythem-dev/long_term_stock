...
$arrLinks      = getSalesWebLinks( 'articlecatalogue', 'FR' );
$arrUser       = getSalesWebUser( 'FR' );

if( !isset( $_REQUEST['FRM_XMLRequest'] ) )
{
    $_REQUEST['FRM_XMLRequest'] = file_get_contents( 'xmldata/articlecatalogue.xml' );
}
else
{
...
<body link="#FFFFFF" vlink="#FFFFFF" alink="#FFFFFF">
	<h1><img src="../images/frankreich.gif" height="5%"/> &nbsp; Articlecatalogue Interface France</h1>
                ...
                <fieldset>
                    <form method="POST" action="<?=$_SERVER['PHP_SELF']?>">
                        ...
                        <table style="font-size: 10pt">
                          ...
                          <tr>
                            <td colspan="2"><? showSubmit( 'FRM_Submit', 'Request Articlecatalogue' ); ?></td>
                          </tr>
                        </table>
                    </form>
                </fieldset>
...
/*
        ?><table align="center" width="100%" style="font-size: 10pt">
            <tr>
                <td>
                    <fieldset>
                        <b>Used Link: &nbsp;</b><font color='black'><?= $_REQUEST['FRM_Link'] ?></font><br/>
                        ...
                        foreach( $arrResult as $x )
                        {
                            ?>
                            <hr>
                            <h2>Nr. <?= $i ?>, Duration: <?= $x['duration'] ?> sec., http Code: <?= $x['httpcode'] ?></h2>
                            <?
                            print htmlspecialchars( $x['content'] );
                            $i++;
                        }
                        ?>
                    </fieldset>
