...
// build query
	...
	if( $logDBName_name == "ArgeOrder" ) $query = $query . ", referenceNumber AS reference ";	
	if( $logDBName_name == "ArgeOrder" ) $query = $query . ", orderEntryNumber AS orderno  ";
...
// query data	
	...
	print sizeof($result)."<hr /><b>Results for Log Server $serverName (".sizeof($result)." entries found)</b>";	
	
	$shown = false;	
	if( $logDBName_name == "CheckOrderBuffer" ) { showOrderResults( $result, $LogDate, $serverName, $logDBName_dbname, $serverLocation );	 $shown = true; }	
	if( $logDBName_name == "ArgeOrder" ) { showOrderResults( $result, $LogDate, $serverName, $logDBName_dbname, $serverLocation ); $shown = true; }
