namespace SalesWebMove
{
    ...
    public class MSV3v1ConfigStrategy : SalesWebConfigStrategy
    {
        public override void loadConfig(string configFileFullName)
        {
            WhatAmI = "MSV3v1";
            ConfigFile = configFileFullName;
            Configuration = PHOENIXUtil.XML.DeSerializeString<msv3v1.Phoenix.Types.Config.SalesWebConfig>(File.ReadAllText(configFileFullName));
        }

        public override void writeConfiguration(string configFileFullName)
        {
            PHOENIXUtil.XML.Serialize(
                Configuration as msv3v1.Phoenix.Types.Config.SalesWebConfig, configFileFullName, true);
        }

        public override void moveBranchesFromSouthToNorth(int[] branches)
        {
            var cfg = Configuration as msv3v1.Phoenix.Types.Config.SalesWebConfig;
