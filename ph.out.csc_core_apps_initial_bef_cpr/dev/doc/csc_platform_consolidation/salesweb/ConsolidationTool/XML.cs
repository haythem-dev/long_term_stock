...
namespace PHOENIXUtil
{
	...
	public static class XML
	{
		...
		public static void Serialize<T>(T toSerialize, string file, bool prettyPrint)
		{
			...
			try
			{
                ...
                if( prettyPrint )
                {
                    ...
                    File.WriteAllText(file, PrettyPrint(xmlContent).Replace("encoding=\"utf-16\"", "encoding=\"utf-8\""));
                }
			}
			catch (Exception Ex)
			{
		...
		public static bool ValidateXML(string xmldata, string xsdfilepath, string targetns = "", bool throwexception = false )
	    {
		    try
		    {
			    ...
			    xmldoc.Validate(PHOENIXUtil.XML.ValidationEventHandler);
			    return true;
		    }
		    catch (Exception ex)
		    {
        ...
        public static string PrettyPrint(string xml)
        {
            ...
            using (var xmlWriter = XmlWriter.Create(stringBldr, settings))
            {
