#! /usr/bin/ksh
 ...
 #Creation-Time:     15:33                                                                                     
 #Purpose:           Starts, stops and status the exemplar binary capresponse for presentation only.           
 ###############################################################################################
 ...
 #--------------------------------------------------------------------#
 readonly BINARY=capresponse.bin
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$umk" == "" ]] || (( 1 != `check_numeric_parameter "$umk"` )) || [[ "$umk" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$umk: >$umk<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 ...
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $UMK_OPTION $umk $BRANCH_OPTION $branch"
 	 else
 		 _parameters="$UMK_OPTION $umk $BRANCH_OPTION $branch";
 	 fi
 ...
 . $(dirname $0)/../vars/capresponse.vars;
