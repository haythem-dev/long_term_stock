#! /usr/bin/ksh
 ...
 #Creation-Time:	09:53
 #Purpose:			starts, stops and status the exemplar binary capdriver for presentation only
 ###############################################################################################
 ...
 readonly SALECENTER_OPTION="-salecenter";
 readonly DB_OPTION="-db";
 readonly LOG_FILENAME_EXTENSION="log";
 readonly LOG_FILENAME_CALLPLAN="callplan";
 ###############################################################################################
 # F U N C T I O N S
 ###############################################################################################
 #====================================#
 ...
 {
 	 if [[ "$2" == "" ]]; then set $1 "15.10.08 09:53:58"; fi
 ...
 function print_local_var
 {
	 ...
	 echo "\$SALECENTER_OPTION:			>$SALECENTER_OPTION<"
 ...
 function eval_parameters
 {
	 ...
	 #--------------------------#
 	 ...
 	 #--------------------------#
 	 if [[ "$salecenter" == "" ]] || (( 1 != `check_numeric_parameter "$salecenter"` )) || [[ "$salecenter" == "0" ]]; then
 		 printStartStop "$STARTMSG"
 		 rc=12;
 		 echo "$PROCNAME: Parameter value missed or invalid value \$salecenter: >$salecenter<!" | tee -a $LOG_RESOURCE
 		 echo ${USAGE} | tee -a $LOG_RESOURCE
 		 printStartStop "$STOPMSG" "with Exit-Code: <${rc}>"
 		 exit ${rc};
 	 fi
 ...
 function assembling_process
 {
	 local _parameters="";
 	 if (( ${#_parameters} > 0 )); then
         ...
         db="`get_ksc_db ${salecenter}`@`get_ksc_infxsrv ${salecenter}`";
 	 if (( ${#_parameters} > 0 )); then
 		 _parameters="${_parameters} $SALECENTER_OPTION $salecenter $DB_OPTION $db"
 	 else
 		 _parameters="$SALECENTER_OPTION $salecenter $DB_OPTION $db";
 	 fi
 ...
 #======================================================#
 if [[ "$start" == "YES" ]]; then
         local readonly LOG_RESOURCE_OLD=${LOG_RESOURCE};
         # unterschiedliche Log-resourcen werden noetig
         LOG_RESOURCE=${LOG_PATH}/${LOG_FILENAME_CALLPLAN}.`get_salescenter_abb ${salecenter}`.${LOG_FILENAME_EXTENSION};
 	 msg=`start_process "${process}" "-NOHUP" "-BG"`
 	 typeset -i _rc_capdriver=$?;
 	 LOG_RESOURCE=${LOG_RESOURCE_OLD}; 
